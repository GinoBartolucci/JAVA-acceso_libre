...
if (ciudadSelect !== null) {
  opcionesCiudad = ciudadSelect.innerHTML;
}
let opcionesLugar;
if (lugarSelect !== null) {
  opcionesLugar = lugarSelect.innerHTML;
}
...
provinciaSelect.addEventListener("change", function() {
    ...
    if(ciudadSelect.disabled){provinciaSelect.remove(0);}
    ciudadSelect.disabled = false;
    ...
    if (provinciaSelect.value === "") {
        ...
        for (let i = lugarSelect.options.length-1; i >= 0; i--) {
            if (lugarSelect.options[i].getAttribute("ciudad_id") !== ciudadId || lugarSelect.options[i].getAttribute("default") == "yes") {
